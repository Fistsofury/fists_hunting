...
Citizen.CreateThread(function()
	...
	local src = source
		if entType == 3 then -- Fur/Skin/Pelt
			-- bear
			if carriedEntityHash == 957520252 then --check for the skin of the animal
				item = 'poor_bear_fur' -- item DB name
				name = 'Poor Bear Fur' -- Display Name
                TriggerEvent('vorp:ShowAdvancedRightNotification', name, "satchel_textures", "tick", "COLOR_PURE_WHITE", 4000) --Notification of your pelt
				TriggerServerEvent("fists_hunting:server:storepelt", item) --trigger the event to add the item to inventory
				DeleteThis(entity) --delete the pelt you are now carrying
			elseif carriedEntityHash == 143941906 then 	-- if not then continue searching for pelt in the list.
				item = 'good_bear_fur'
				name = 'Good Bear Fur'
